[Manual](README.md) ❭ Requirements

<div align="center">

  <picture>
    <source media="(prefers-color-scheme: dark)" srcset="../../.github/logo/dark/256x173/Man.png">
    <source media="(prefers-color-scheme: light)" srcset="../../.github/logo/light/256x173/Man.png">
    <img alt="Fallback image description" src="../../.github/logo/light/256x173/Man.png">
  </picture>

  ![RELEASE](https://img.shields.io/badge/Release-26.8-teal)

<h1>Requirements</h1>

</div>

You'll need a few things to use the Tingen Web Service. Don't worry, we'll walk through everything step by step.

What you'll need:

* A host machine
* The latest Community Release of the Tingen Web Service

NEXT: [Setup the Host Machine](SetupHost.md)

<br/>

***

[The Tingen Web Service Manual](README.md) ❭ Requirements

<sub>Last updated: 260709</sub>
