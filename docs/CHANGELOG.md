❰ [Back to repository main page](../README.md)

<div align="center">
  <img src="../.github/logo/Tingen-WebService-256x173.png" alt="Tingen Web Service">
  <h1>Changelog</h1>
</div>

## R26.8

> Please see the [release notes](./release-notes/R26.8-release-notes.md) for more details.

* New
  * None
* Updated
  * None
* Fixed
  * None
* Removed
  * None
* Security
  * None

## R26.7 (2026-07-24)

> Please see the [release notes](./release-notes/R26.7-release-notes.md) for more details.

This release was a major refactor.

***

## R26.6 (2026-07-24)

> Please see the [release notes](./release-notes/R26.6-release-notes.md) for more details.

* New
  * Tingen Web Service Manual
  * Tingen Web Service Development Manual
  * Tingen Web Service API Reference
  * Tingen Web Service Glossary
  * The MyAvatar Development Community documentation archives
* Updated
  * Project XML documentation
* Fixed
  * None
* Removed
  * None
* Security
  * None

***

## R26.5 (2026-07-24)

> There are no release notes for R26.5.

* Updated
  * Project XML documentation

***

## R26.4 (2026-07-24)

> There are no release notes for R26.4.

* New
  * None
* Updated
  * Project XML documentation
* Fixed
  * None
* Removed
  * None
* Security
  * None

***

## R26.3 (2026-07-24)

> There are no release notes for R26.3.

* Updated
  * Project XML documentation

***

## R26.2 (2026-07-24)

> There are no release notes for R26.2.

* Updated
  * Project XML documentation

***

## R26.1 (2026-07-24)

> There are no release notes for R26.1.

* New
  * None
* Updated
  * Project XML documentation
* Fixed
  * None
* Removed
  * None
* Security
  * None

***

## R26.0 (2026-07-24)

> Please see the [release notes](./release-notes/R26.0-release-notes.md) for more details.

* New
  * None.
* Updated
  * Sandcastle Help File Builder
  * ScriptLink Standard
* Fixed
  * None
* Removed
  * None
* Security
  * None

***

## R25.12 (2025-12-14)

> There are no release notes for R25.12.

This release was a year-end wrap up without any changes.

***

## R25.11 (2025-11-28)

> Please see the [release notes](./release-notes/R25.11-release-notes.md) for more details.

* New
  * Query UserId/User Roles  
  * Catch and export OptionObjects
* Updated
  * None
* Fixed
  * None
* Removed
  * None
* Security
  * None

***

## R25.10 (2025-10-24)

> Please see the [release notes](./release-notes/R25.10-release-notes.md) for more details.

* New
  * Daily maintenance function  
  * Default ScriptLink events for modules
* Updated
  * Translation tables functionality
  * Module.OpenIncident
* Fixed
  * None
* Removed
  * None
* Security
  * None

***

## R25.9 (2025-09-26)

> Please see the [release notes](./release-notes/R25.9-release-notes.md) for more details.

* New
  * web.config settings
  * Module.OpenIncident
* Updated
  * Session data now includes the current day/time, the Avatar UserName, and framework details
* Fixed
  * None
* Removed
  * AdminMode
  * The popup that appears when the Tingen Web Service is disabled.
* Security
  * None

***

## R25.8 (2025-08-22)

> Please see the [release notes](./release-notes/R25.8-release-notes.md) for more details.

* New
  * AdminMode 
* Updated
  * Logging functionality to include additional types of logs
* Fixed
  * None
* Removed
  * None
* Security
  * None
  
***

> [!INFO]  
> The Tingen Web Service has gone through *many* iterations since 2018, and unfortunately, I don't have detailed release notes for the earlier versions.

***

❰ [Back to repository main page](../README.md)
